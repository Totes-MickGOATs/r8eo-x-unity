...
## TDD Cycle (Red-Green-Commit) — MANDATORY

Every bug fix or feature follows this exact cycle. No steps may be skipped.

1. **Hypothesize** — Identify the potential cause or desired behavior
2. **Write a failing test** — Confirms the hypothesis
3. **Run the test → confirm RED** — Execute it. If it passes, hypothesis is wrong — revise
4. **Implement the fix/feature** — Minimum code to pass
5. **Run the test → confirm GREEN** — Execute it. If it fails, iterate on the fix
6. **Commit**

> **CRITICAL:** Steps 3 and 5 are non-negotiable. A test that was never run proves nothing. An agent that skips test execution and claims something is "fixed" or "verified" is violating the project's core practice.

### Unit vs Integration Tests

Most changes need BOTH:

- **Unit tests** — Single function/class in isolation. Always required.
- **Integration tests** — Multiple systems working together (signals, scene tree wiring, autoload interaction). Required when the change involves cross-system behavior.
- A passing unit test does NOT mean the feature works at runtime. If systems are wired together, test the wiring too.

### Running Tests During TDD

```bash
# Run specific test (use during red/green cycle)
godot --headless -s addons/gut/gut_cmdln.gd -gtest=res://tests/test_<system>.gd
...
- [reverse-engineering](../reverse-engineering/SKILL.md) — Chain-of-custody debugging: trace from symptom to root cause, then use TDD to verify the fix
- [user-interaction-testing](../user-interaction-testing/SKILL.md) — Input injection, GutInputSender, E2E user journey testing, menu flow simulation
