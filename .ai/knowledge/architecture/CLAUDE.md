# knowledge/architecture/

Architecture documentation, ADRs, coding standards, and system design guides.

## Files

| File | Role |
|------|------|
| `system-overview.md` | High-level architecture: scene graph, signal map, data flow |
| `coding-standards.md` | Project coding conventions and style guide |
| `adr-TEMPLATE.md` | Template for Architecture Decision Records |
| `adr-001-physics-model.md` | ADR: Raycast-based wheel physics with Hooke's law suspension |
| `subagent-patterns.md` | Patterns for multi-agent workflows |
| `e2e-testing-guide.md` | End-to-end testing architecture and patterns |
